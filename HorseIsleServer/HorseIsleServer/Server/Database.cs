...
namespace HISP.Server
{
    public class Database
    {
        ...
        public static void RunSql(string statement)
        {
        ...
        {
            ConnectionString = "server=" + ConfigReader.DatabaseIP + ";user=" + ConfigReader.DatabaseUsername + ";password=" + ConfigReader.DatabasePassword + ";database=" + ConfigReader.DatabaseName + ";AllowLoadLocalInfile=true";
            using (MySqlConnection db = new MySqlConnection(ConnectionString))
            {
                ...
                RunSql(ItemQueue);
                RunSql(SolvedRealTimeRiddles);
                RunSql(MutedPlayers);

                RunSql(MessageQueue);
                RunSql(SolvedRealTimeRiddle);
                RunSql(DressupRooms);

                RunSql(AuctionTable);
                RunSql(NpcPos);

                RunSql(RiddlesComplete);
                RunSql(Ranches);
                RunSql(Treasure);

                RunSql(SavedDrawings);
                RunSql(DrawingRooms);

                RunSql(TrackingStats);
                ...
                RunSql(Awards);
                RunSql(DroppedItems);
                RunSql(InventoryTable);
                RunSql(ShopInventory);
                RunSql(TrackedQuest);
                RunSql(PoetryRooms);
                RunSql(BannedPlayers);
                RunSql(CompetitionGear);
                RunSql(NpcStartPoint);
                RunSql(LastPlayer);
        ...
        {
            List<Item.ItemPurchaseQueueItem> queueItems = new();
            ...
            while (reader.Read())
            {
                Item.ItemPurchaseQueueItem itm = new();
        ...
        {
            List<Dressup.DressupPeice> peices = new();
            ...
            while (reader.Read())
            {
                ...
                Dressup.DressupPeice peice = new(room, peiceId, x, y, active, false);
                peices.Add(peice);
            }
        ...
        public static void SetRanchBuilding(int ranchId, int buildingNumber, int buildingId)
        {
            using MySqlConnection db = Connect();
            MySqlCommand sqlCommand = db.CreateCommand();
            sqlCommand.CommandText = "UPDATE Ranches SET building@buildingNumber=@buildingId WHERE ranchId=@ranchId";
            sqlCommand.Parameters.AddWithValue("@ranchId", ranchId);
        ...
        public static int GetRanchBuilding(int ranchId, int buildingNumber)
        {
            using MySqlConnection db = Connect();
            MySqlCommand sqlCommand = db.CreateCommand();
            sqlCommand.CommandText = "SELECT building@buildingNumber FROM Ranches WHERE ranchId=@ranchId";
            sqlCommand.Parameters.AddWithValue("@ranchId", ranchId);
            sqlCommand.Parameters.AddWithValue("@buildingNumber", buildingNumber);
            sqlCommand.Prepare();
        ...
        public static Treasure[] GetTreasures()
        {
            ...
            while (reader.Read())
            {
                ...
                Treasure treasure = new(x, y, type, randomId, value);
        ...
        public static void LoadWildHorses()
        {
            ...
            while (reader.Read())
            {
                ...
                HorseInfo.Breed horseBreed = HorseInfo.GetBreedById(breedId);
                HorseInstance inst = new(horseBreed, randomId)
                {
                ...
                new WildHorse(inst, x, y, timeout, false);
            }
        }
        ...
        {
            if (!LastPlayerExist(roomId))
                AddLastPlayer(roomId, -1);
        ...
        public static HorseInstance[] GetPlayerHorsesInCategory(int playerId, string category)
        {
            ...
            List<HorseInstance> instances = new();
        ...
        {
            if(CheckTrackedQuestExists(playerId, questId))
            {
                using MySqlConnection db = Connect();
                MySqlCommand sqlCommand = db.CreateCommand();
        ...
        public static bool CheckTrackedQuestExists(int playerId, int questId)
        {
            using MySqlConnection db = Connect();
            MySqlCommand sqlCommand = db.CreateCommand();
        ...
        {
            List<TrackedQuest> TrackedQuests = new();
            ...
            while(reader.Read())
            {
                TrackedQuest TrackedQuest = new(playerId, reader.GetInt32(0), reader.GetInt32(1));
        ...
        {
            List<DroppedItems.DroppedItem> itemList = new();
        ...
        public static void AddDroppedItemBatch(DroppedItems.DroppedItem[] items)
        {
            using MySqlConnection db = Connect();

            DataTable table = new DataTable();
            table.TableName = "DroppedItems";
            table.Columns.Add("X", typeof(int));
            table.Columns.Add("Y", typeof(int));
            table.Columns.Add("RandomID", typeof(int));
            table.Columns.Add("ItemId", typeof(int));
            table.Columns.Add("DespawnTimer", typeof(int));
            ...
            foreach(var item in items)
            {
                ...
                row["X"] = item.X;
                row["Y"] = item.Y;
                row["RandomId"] = item.Instance.RandomId;
                row["ItemId"] = item.Instance.ItemId;
                row["DespawnTimer"] = item.DespawnTimer;
                row["Data"] = item.Data;

                table.Rows.Add(row);
            };

            var copy = new MySqlBulkCopy(db);
        ...
        {
            List<Mailbox.Mail> mailList = new();
        ...
        public static Highscore.HighscoreTableEntry[] GetPlayerHighScores(int playerId)
        {
            ...
            while (reader.Read())
            {
                Highscore.HighscoreTableEntry highscoreEntry = new();
        ...
        public static void DoIntrestPayments(int intrestRate)
        {
            if (intrestRate == 0)
            {
                Logger.WarnPrint("Intrest rate is 0, as dividing by 0 causes the universe to implode, adding intrest has been skipped.");
                return;
            }
